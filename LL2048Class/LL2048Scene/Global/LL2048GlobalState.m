...
@property (nonatomic, readwrite) LL2048GameType gameType;
@property (nonatomic) NSInteger theme;

@end


@implementation LL2048GlobalState

+ (LL2048GlobalState *)state
{
  static LL2048GlobalState *state = nil;
  
  static dispatch_once_t once;
  dispatch_once(&once, ^{
    state = [[LL2048GlobalState alloc] init];
  });
...
{
  if (self.gameType == LL2048GameTypeFibonacci) {
...
- (NSInteger)valueForLevel:(NSInteger)level
{
  if (self.gameType == LL2048GameTypeFibonacci) {
    ...
    NSInteger value = 1;
    NSInteger base = self.gameType == LL2048GameTypePowerOf2 ? 2 : 3;
    for (NSInteger i = 0; i < level; i++) {
      value *= base;
    }
    return value;
...
{
  return [[LL2048Theme themeClassForType:self.theme] colorForLevel:level];
}


- (UIColor *)textColorForLevel:(NSInteger)level
{
  return [[LL2048Theme themeClassForType:self.theme] textColorForLevel:level];
}
...
{
  return [[LL2048Theme themeClassForType:self.theme] backgroundColor];
}


- (UIColor *)scoreBoardColor
{
  return [[LL2048Theme themeClassForType:self.theme] scoreBoardColor];
}


- (UIColor *)boardColor
{
  return [[LL2048Theme themeClassForType:self.theme] boardColor];
}


- (UIColor *)buttonColor
{
  return [[LL2048Theme themeClassForType:self.theme] buttonColor];
}


- (NSString *)boldFontName
{
  return [[LL2048Theme themeClassForType:self.theme] boldFontName];
}


- (NSString *)regularFontName
{
  return [[LL2048Theme themeClassForType:self.theme] regularFontName];
}
