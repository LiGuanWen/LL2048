...
- (id)initWithSize:(CGSize)size{
  if (self = [super initWithSize:size]) {
    _manager = [[LL2048GameManager alloc] init];
  }
  return self;
}

- (void)loadBoardWithGrid:(LL2048Grid *)grid{
  UIImage *image = [LL2048GridView gridImageWithGrid:grid];
  ...
  [board setScale:0.5];
//   board.size = CGSizeMake(CGRectGetWidth(self.frame), CGRectGetHeight(self.frame));
...
- (void)commitTranslation:(CGPoint)translation
{
  ...
  if (absX > absY * VALID_SWIPE_DIRECTION_THRESHOLD) {
    translation.x < 0 ? [_manager moveToDirection:LL2048DirectionLeft] :
                        [_manager moveToDirection:LL2048DirectionRight];
  } else if (absY > absX * VALID_SWIPE_DIRECTION_THRESHOLD) {
    translation.y < 0 ? [_manager moveToDirection:LL2048DirectionUp] :
                        [_manager moveToDirection:LL2048DirectionDown];
  }
